...
ip_route_add(ip_addr_t network, ip_addr_t netmask, ip_addr_t nexthop,
             struct ip_iface *iface)
{
  ...
  if (nexthop != IP_ADDR_ANY) {
    infof(
      "%s/%s via %s dev %s src %s", ip_addr_ntop(network, addr1, sizeof(addr1)),
      ip_addr_ntop(netmask, addr2, sizeof(addr2)),
      ip_addr_ntop(nexthop, addr3, sizeof(addr3)), NET_IFACE(iface)->dev->name,
...
static int
ip_output_device(struct ip_iface *iface, const uint8_t *data, size_t len,
                 ip_addr_t target)
{
  char addr[IP_ADDR_STR_LEN];
...
ip_build_packet(uint8_t protocol, const uint8_t *data, size_t len, uint16_t id,
                uint16_t offset, ip_addr_t src, ip_addr_t dst, uint8_t *buf,
                size_t size)
{
  uint16_t hlen, total;
  struct ip_hdr *hdr;
...
ssize_t
ip_output(uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src,
          ip_addr_t dst)
{
  char addr1[IP_ADDR_STR_LEN];
  char addr2[IP_ADDR_STR_LEN];
